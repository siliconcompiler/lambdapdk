...
set met4_pitch [expr {([lindex [ord::get_core_area] 3] - [lindex [ord::get_core_area] 1]) / 2}]
if {$met4_pitch > 75.6} {
    set met4_pitch 75.6
}
set met5_pitch [expr {([lindex [ord::get_core_area] 2] - [lindex [ord::get_core_area] 0]) / 2}]
if {$met5_pitch > 75.6} {
    set met5_pitch 75.6
}
set top1_pitch [expr {([lindex [ord::get_core_area] 3] - [lindex [ord::get_core_area] 1]) / 2}]
if {$top1_pitch > 75.6} {
    set top1_pitch 75.6
}
set top2_pitch [expr {([lindex [ord::get_core_area] 2] - [lindex [ord::get_core_area] 0]) / 2}]
if {$top2_pitch > 75.6} {
    set top2_pitch 75.6
}
...
add_pdn_stripe -grid {grid} -layer {Metal4} -width {2.200} -pitch [snap_grid $met4_pitch] \
    -offset [snap_grid [expr {$met4_pitch / 2}]]
